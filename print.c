...
void prt_char(char*,int);
void prt_float(float*,int);
...
void prt_expression(A_NODE *node, int s)
{
	...
	switch(node->name) {
	   ...
	   case N_EXP_FLOAT_CONST :
 		prt_float((float*)(node->clink), s+1);
		break;
	   case N_EXP_CHAR_CONST :
 		prt_char((char*)node->clink, s+1);
		break;
	...
	if (t==int_type) 
	   ...
	   switch (t->kind) {
		case T_ENUM:
			...
			prt_A_ID_LIST((A_ID*)t->field,s+2);
			break;
		case T_POINTER:
			t->prt=TRUE;
			printf("POINTER\n");
...
char *spec_name[]={"NULL","AUTO","STATIC","TYPEDEF","REGISTER","EXTERN"};

void prt_A_ID_NAME(A_ID *id, int s)
{
	print_space(s);
	printf("(ID=\"%s\") TYPE:%p KIND:%s SPEC=%s LEV=%d \n", id->name, id->type,
